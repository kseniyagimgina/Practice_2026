...
public class AttributeReflectionTests
{
    ...
    public void Class_PrintTypeInfo()
    {
        ...
        Assert.Contains("SampleClass", output.ToString());
        Assert.Contains("Пример класса", output.ToString());
        Assert.Contains("Version: 1.0", output.ToString());
        Assert.Contains("TestMethod", output.ToString());
        Assert.Contains("Тестовый метод", output.ToString());
        Assert.Contains("Number", output.ToString());
        Assert.Contains("Числовое свойство", output.ToString());
    }
}
